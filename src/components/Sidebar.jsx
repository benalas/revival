      ...
      <aside className={`fixed top-0 left-0 h-full w-60 bg-forest-700 z-30 flex flex-col transition-transform duration-200 ${open ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 lg:static lg:z-auto`}>
            ...
            <NavLink key={to} to={to} end={to === '/'} onClick={onClose}
              className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all ${
                  isActive
                    ? 'bg-forest-500/60 text-cream-50'
                    : 'text-forest-300 hover:bg-forest-600/40 hover:text-cream-50'
                }`
              }>
              <Icon size={16} />
              {label}
        ...
        <div className="px-4 py-5 border-t border-forest-600/50">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-8 h-8 rounded-full bg-forest-500 flex items-center justify-center text-cream-50 font-display text-sm flex-shrink-0">B</div>
            <div className="min-w-0">
              <p className="text-sm font-medium text-cream-50 truncate">Benjamin Alas</p>
              <p className="text-xs text-forest-400 truncate">balas@nmbnow.com</p>
            </div>
          </div>
